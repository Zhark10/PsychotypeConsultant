...
export const UserActionHandler = async (bot) => {
  ...
  bot.on("callback_query", async (callbackQuery) => {
    ...
    const updatedUser = await User.findOneAndUpdate(user, { $push: { testAnswers: answerId } }, { new: true })

    if (updatedUser.testAnswers.length >= test.length) {
      const message = 'Тест завершен! Спасибо за участие! После проверки результатов с вами свяжутся!'
      const createdPicture = await templateService.createTemplateForSimpleMessage(message)
